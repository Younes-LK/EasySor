                            ...
                            @foreach ($contract->equipments as $index => $contractEquipment)
                                <div class="p-3 mb-2 border dark:border-gray-700 rounded-md equipment-item"
                                    ...
                                    <p class="font-semibold dark:text-indigo-400">
                                        {{ $contractEquipment->equipment->name ?? 'تجهیز نامشخص' }} (موجودی:
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (customerSelect) {
                customerSelect.addEventListener('change', (e) => {
                    populateAddresses(e.target.value);
                });
            }
                ...
                addEquipmentBtn.addEventListener('click', () => {
                    ...
                    <div class="grid md:grid-cols-4 gap-3">
                        <div class="md:col-span-2">
                            ...
                            <select name="new_equipments[${newEquipmentIndex}][equipment_id]" class="mt-1 block w-full text-sm rounded border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white new-equipment-select">
                                ${equipmentOptions}
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs dark:text-gray-300">تعداد</label>
                            <input type="number" name="new_equipments[${newEquipmentIndex}][quantity]" value="1" min="1" class="mt-1 block w-full text-sm rounded border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white">
                        </div>
                        <div>
                            <label class="block text-xs dark:text-gray-300">قیمت واحد</label>
                            <input type="number" name="new_equipments[${newEquipmentIndex}][unit_price]" value="0" min="0" class="mt-1 block w-full text-sm rounded border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white new-equipment-price">
                        </div>
                        <div class="md:col-span-4">
                            <label class="block text-xs dark:text-gray-300">یادداشت</label>
            ...
            // Update the generic remove button listener to also handle new logs
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('remove-new-item-btn')) {
                    const itemToRemove = event.target.closest(
                        '.new-equipment-item, .new-payment-item, .new-log-item');
                    if (itemToRemove) {
                        itemToRemove.remove();
                    }
                }
