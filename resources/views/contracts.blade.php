...
@section('content')
    ...
    <div class="container mx-auto px-4 py-6">
                ...
                @if ($editMode && $contract->payments->count() > 0)
                    @foreach ($contract->payments as $index => $payment)
                        <div class="p-3 mb-2 border dark:border-gray-700 rounded-md payment-item"
                            ...
                            <div class="grid md:grid-cols-4 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="payments[{{ $index }}][amount]"
                                        value="{{ old("payments.{$index}.amount", $payment->amount) }}" min="0"
                                        class="mt-1 block w-full text-sm rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                </div>
                                <div>
                            ...
                            <label class="flex items-center mt-2 text-sm">
                                <input type="checkbox" name="payments[{{ $index }}][_remove]" value="1"
                                    class="rounded border-gray-300 dark:border-gray-600 text-red-600 shadow-sm focus:ring-red-500">
        ...
        <div class="mt-8 pt-6 border-t dark:border-gray-700">
            <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 text-base"
                id="submit-btn">
                            ...
                            <td class="p-3">
                                @php
                                    ...
                                    $statusColor = match ($contractItem->status) {
                                        ...
                                        'completed' => 'bg-blue-200 text-blue-800 dark:bg-blue-700 dark:text-blue-100',
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // ✅ CORRECTED: Select the new parent wrapper you added in the HTML
            ...
            // ✅ CORRECTED: The listener is now on the main wrapper to catch events from NEW and EXISTING items.
